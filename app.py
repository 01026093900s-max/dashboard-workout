...
<style>
    ...
    @media (prefers-color-scheme: dark) {
        ...
        .kpi-card, .top3-card, .cumulative-card { background: #171a22 !important; border-color: #2c3342 !important; box-shadow: none !important; }
        .kpi-value, .kpi-unit { color: #f3f4f6 !important; }
        .top3-item, .cum-rank-row { border-bottom-color: #2c3342 !important; }
        .cum-rank-name { color: #f3f4f6 !important; }
        .cum-rank-bar-wrap { background: #2c3342 !important; }
    }
</style>
...
def _fig_week_compare_lines(
    ...
    """두 주의 일별 인증 수 라인 차트. highlight_today=True면 이번주 오늘 점 강조."""
...
def _fig_archive_week_lines(merged_this_week, merged_prev_week, sun_d):
    """아카이브 주: 해당 주 vs 전주 일별 인증 라인 (완료된 7일 전부 표시)."""
    week_dates = [sun_d + timedelta(days=i) for i in range(7)]
...
def _resolve_prev_week_table_rows(archive_list, cafe_rows, manual_certs, sun_d):
    """아카이브 그래프용 전주 table_rows (아카이브 → 라이브 순으로 조회)."""
    prev_sun = sun_d - timedelta(days=7)
    prev_sat = prev_sun + timedelta(days=6)
    for entry in archive_list or []:
        if entry.get("week_sun") == prev_sun.isoformat():
            live_p = _build_table_rows_for_week(cafe_rows, manual_certs, prev_sun, prev_sat)
            return merge_live_and_snapshot_week_rows(live_p, entry.get("table_rows") or [])
    return _build_table_rows_for_week(cafe_rows, manual_certs, prev_sun, prev_sat)
    ...
    if not archive:
        ...
        for idx, entry in enumerate(reversed(archive)):
            ...
            with st.expander(f"📅 {period_label}", expanded=outer_expanded):
                ...
                _prev_tr = _resolve_prev_week_table_rows(
                    archive, cafe_rows, st.session_state.get("manual_certs", []), sun_d
                )
                st.markdown(
                ...
                _fig_arch = _fig_archive_week_lines(merged_w, _prev_tr, sun_d)
                st.plotly_chart(_fig_arch, width="stretch", key=f"archive_graph_{week_sun_s}")
                if _ama:
                    st.caption(
...
st.caption("데이터는 카페에서 수집된 내용이며, 새로고침 시 다시 가져옵니다. 주간 기준은 일요일~토요일이며, 매주 일요일 00:00에 새 주로 전환됩니다.")
